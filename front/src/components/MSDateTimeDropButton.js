...
export default function MSDateTimeDropButton() {
  const [date, setDate] = React.useState();
  const [time, setTime] = React.useState('');
  ...
  return (
      <Box align="left" pad="none" width="fit-content" >
        <DropButton
          primary
          open={open}
        ...
        >
          <Box direction="row" gap="medium" align="center" pad="10px 20px" width="fit-content" >
            <Text size="small" pad="none" color={date ? undefined : 'light-1'}>
              {date
                ? `${new Date(date).toLocaleDateString()} ${time}`
                : 'Select date & time'}
            </Text>
            <Schedule size="20px" color="light-1" />
          </Box>
        </DropButton>
      </Box>
  );
